                            ...
                            <div class="row m-0 p-2">
                                ...
                                <div class="col-sm-12 col-md-6">
                                    <div class="d-flex justify-content-end">
                                        <form action="{{ route('admin.variations.index') }}" method="GET">
                            ...
                            <table class="table table-hover text-nowrap border-top">
                                <thead class="border-top">
                                    <tr>
                                        <th>Sl no.</th>
                                        <th>
                                            <a class="sortable-link" href="{{ route('admin.variations.trash', ['query' => $query, 'sort_by' => 'name', 'sort_order' => ($sort_by == 'name' && $sort_order == 'asc') ? 'desc' : 'asc']) }}">
                                ...
                                <tbody>
                                    @foreach($trashedVariations as $variation)
                                        <tr>
                                            <td>{{$trashedVariations->firstItem() + $loop->index}}</td>
                                            <td>{{ $variation->name }}</td>

                                            <td>{{ $variation->deleted_at }}</td>
                                            <td>
                                                <x-actions.restore-btn route="admin.variations.untrash" label="Restore" :route-params="[$variation->id]" />
                                                <x-actions.delete-btn route="admin.variations.forceDelete" label="Delete" :route-params="[$variation->id]" alertTitle="Froce delete {{$variation->name}}"/>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
